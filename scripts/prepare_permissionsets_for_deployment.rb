# Note: The lazy author just took the prepare-profiles-for-deployment.rb
# script and replaced some keywords (notably "profile" with "permission set")
# where applicable to repurpose the script for preparing permission sets
#
# When reading the documentation, please note that any mention of the word
# "profile" can likely be substituted for the word "permission set"
...
PERMISSION_SET_FILE_EXTENSION = ".permissionset"
...
knowns = {
    ...
    CUSTOM_TAB_XML_NAME => [],
    FIELDS_FIELD_NAME => [],
    # Page Layout assignments are not available with permission sets
    #LAYOUT_XML_NAME => [],
...
permission_sets = {}    # Prepared profiles we're going to repackage
log_rows = [
    [
        "Permission Set Name", 
        "XML Field Name", 
...
Zip::ZipFile.foreach(source_zip_path) do |zip_entry|
    ...
    if zip_entry.name.end_with? PERMISSION_SET_FILE_EXTENSION
        debug "Preparing: #{zip_entry.name}"
        
        # Parse the profile name to use as the key for our profiles hash
        
        permission_set_name = zip_entry.name.split("/").last.split(".").first
        
        # Let's try REXML again... for ease of dealing with namespaces
        
        debug "Constructing document from XML..."
        permission_set_doc = REXML::Document.new zip_entry.get_input_stream
        permission_sets[permission_set_name] = permission_set_doc
        ...
        permission_set_doc.each_element("PermissionSet/#{xml_field_name}/application") do
            |el|
        ...
        debug "Processing #{xml_field_name}..."
        permission_set_doc.each_element("PermissionSet/#{xml_field_name}/apexClass") do
            |el|
        ...
        debug "Processing #{xml_field_name}..."
        permission_set_doc.each_element("PermissionSet/#{xml_field_name}/field") do
            |el|
        ...
        # No Step 4 for removing unrecognized layoutAssignments elements.
        # Page Layout assignments are not available with permission sets
        ...
        permission_set_doc.each_element("PermissionSet/#{xml_field_name}/object") do
            |el|
        ...
        remaining_knowns.each do |component_name|
            permission_set_el = permission_set_doc.root
            ...
            permission_set_el.insert_after last_el, xml_field_el
        ...
        debug "Processing #{xml_field_name}..."
        permission_set_doc.each_element("PermissionSet/#{xml_field_name}/apexPage") do
            |el|
        ...
        permission_set_doc.each_element("PermissionSet/#{xml_field_name}/recordType") do
            |el|
        ...
        xml_field_name = "tabSettings"
        debug "Processing #{xml_field_name}..."
        
        remaining_knowns = knowns[CUSTOM_TAB_XML_NAME].clone
        last_el = nil  # The last found element for this XML field
        
        permission_set_doc.each_element("PermissionSet/#{xml_field_name}/tab") do
            |el|
        ...
        remaining_knowns.each do |component_name|
            ...
            log_rows.push [
                ...
                component_name,
                "Add",
                "Not in #{destination_zip_path}; chmod None"
            ]
        end
        ...
        permission_set_doc.each_element("PermissionSet/#{xml_field_name}/name") do
            |el|
    ...
    end # zip_entry.name.end_with? PERMISSION_SET_FILE_EXTENSION
end
...
permission_sets.each do |permission_set_name, permission_set|
    output_zip_file.get_output_stream("permissionsets/#{permission_set_name}.permissionset") do |f|
        permission_set.write(f)
    end
end
...
permission_set_types_el = REXML::Element.new("types", package_el)

permission_sets.keys.each do |permission_set_name|
    permission_set_members_el = REXML::Element.new("members", permission_set_types_el)
    permission_set_members_el.add_text permission_set_name
end

permission_set_types_name_el = REXML::Element.new("name", permission_set_types_el)
permission_set_types_name_el.add_text "PermissionSet"
